...
function FindingDetail({ finding, rulesetVersion, hasIssues }) {
  if (!finding) return <aside className="issue-detail"><span className="detail-status severity-info">Готово</span><h3>{hasIssues ? 'В этом фильтре замечаний нет' : 'Замечаний не найдено'}</h3><p>{hasIssues ? 'Выберите другой фильтр, чтобы увидеть найденные замечания.' : `Сверка применяла только структурные правила версии ${rulesetVersion}.`}</p></aside>

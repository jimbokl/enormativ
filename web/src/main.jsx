...
import { openTableFile } from './lib/delimited.js'
import { MAX_FILE_BYTES, previewWorksheet, sha256, worksheetColumns } from './lib/workbook.js'
...
function issueCsv(result) {
  const rows = [['Уровень', 'Код', 'Файл', 'SHA-256', 'Лист', 'Ячейка', 'Строка текста', 'Исходное значение', 'Второй файл', 'SHA-256 второго файла', 'Лист второго файла', 'Ячейка второго файла', 'Строка второго текста', 'Значение второго файла', 'Объединена с', 'Сущность', 'Замечание', 'Действие', 'Основание', 'Версия правила']]
  for (const finding of result.issues) {
    rows.push([
      ...
      finding.location?.cells?.join(' / ') || finding.location?.cell, finding.location?.sourceLine, finding.observed, finding.relatedLocation?.fileName, finding.relatedLocation?.fileHash,
      finding.relatedLocation?.sheet, finding.relatedLocation?.cells?.join(' / ') || finding.relatedLocation?.cell, finding.relatedLocation?.sourceLine, finding.relatedObserved,
...
function SiteHeader({ root = './', active = '' }) {
  return <header className="site-header" id="top"><div className="container header-inner"><Brand root={root} /><nav aria-label="Основная навигация"><a href={`${root}#catalog`}>Каталог</a><a href={`${root}tools/check/`} aria-current={active === 'check' ? 'page' : undefined}>Проверить таблицу</a><a href={`${root}tools/compare/`} aria-current={active === 'compare' ? 'page' : undefined}>Сравнить файлы</a></nav><a className="header-action" href={active ? `${root}#catalog` : `${root}tools/check/`}>{active ? 'К каталогу' : 'Открыть проверку'} <ArrowRight size={16} /></a></div></header>
}
...
function ComparisonPanel({ inputRef, parsed, file, config, confirmed, busy, onLoad, onRemove, onMapping, onConfirm }) {
  return <section className="comparison-panel" aria-label="Сверка двух файлов">
    <div className="comparison-heading"><div><span className="eyebrow">Шаг 02 · второй файл</span><h3>Сопоставить два файла</h3><p>Сверим номера и координаты источников между XLSX, CSV или TSV. Изменения в документах могут быть обоснованными; решение остаётся за экологом.</p></div></div>
    {!parsed ? <label className="comparison-picker"><FileSpreadsheet size={19} /><span>Добавить второй файл с источниками</span><input ref={inputRef} type="file" accept=".xlsx,.csv,.tsv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv,text/tab-separated-values" disabled={busy} onChange={event => onLoad(event.target.files[0])} /></label> : <>
...
function FindingDetail({ finding, rulesetVersion, hasIssues, hasUncertain, isComparison }) {
  ...
  return <aside className="issue-detail">
    ...
    <div className="detail-block"><span>Исходное значение</span><p>{finding.observed || 'Пусто'}{finding.mergedFrom ? ` · объединена с ${finding.mergedFrom}` : ''}{finding.location?.sourceLine ? ` · строка текста ${finding.location.sourceLine}` : ''}</p></div>
...
function ToolPage({ mode }) {
  ...
  async function loadFile(file) {
    ...
    try {
      if (!/\.(xlsx|csv|tsv)$/i.test(file.name)) throw new Error('Выберите XLSX, CSV или TSV. Файлы XLS и PDF пока не проверяются.')
      if (file.size > MAX_FILE_BYTES) throw new Error('Файл больше 20 МБ. Для первой версии выберите меньшую книгу.')
      const bytes = await file.arrayBuffer()
      const hash = await sha256(bytes)
      const opened = await openTableFile(bytes, file.name)
      setParsed(opened)
  ...
  async function loadComparisonFile(file) {
    ...
    setError(''); setResult(null); setSelectedIssue(null); setBusy(true)
    try {
      if (!/\.(xlsx|csv|tsv)$/i.test(file.name)) throw new Error('Второй файл должен быть XLSX, CSV или TSV.')
  ...
  function runValidation() {
    ...
    try {
      ...
      if (mode === 'compare') {
        if (!comparisonParsed) throw new Error('Добавьте второй файл для сравнения.')
    ...
    <main>
      <section className="tool-intro"><div className="container tool-intro-inner">
        <div><a className="breadcrumb" href="../../#catalog">← Каталог проверок</a><span className="eyebrow">Каталог / Экология / {isCompare ? 'Сопоставление' : 'Структурная проверка'}</span>
          <h1>{isCompare ? 'Сравнение таблиц источников выбросов — XLSX и CSV' : 'Проверка таблицы источников выбросов — XLSX и CSV'}</h1>
          <p>{isCompare ? 'Найдите расхождения номеров и координат между двумя XLSX, CSV или TSV. Сначала подтвердите, что данные относятся к одному объекту и сопоставимому состоянию.' : 'Откройте XLSX, CSV или TSV, укажите колонки и получите замечания с адресами ячеек. Проверка выполняется локально в браузере.'}</p>
      ...
      <section className="checker-section section" id="checker"><div className="container">
        <div className="checker-title"><div><span className="eyebrow">{isCompare ? '01 / Первый файл' : 'Рабочая область'}</span><h2>{canProcessFiles === false ? 'Проверка временно недоступна' : isCompare ? 'Подготовьте данные' : 'Загрузите файл'}</h2><p>{canProcessFiles === false ? 'Как только для домена заработает защищённое соединение, здесь снова можно будет открыть файл.' : isCompare ? 'Загрузите первый файл, затем добавьте второй и подтвердите сопоставимость. Лист выбросов в первом XLSX можно проверить дополнительно.' : 'Начните с таблицы источников. Отдельный лист выбросов в XLSX можно добавить для проверки связей.'}</p></div><span className="beta-label"><span></span> Ранняя версия</span></div>
        <div className="checker-shell">
        <div className="checker-topline"><div>{canProcessFiles === true ? <><span className="live-dot"></span> Локальный режим</> : canProcessFiles === false ? <><CircleAlert size={15} /> Ожидаем HTTPS</> : 'Проверяем доступность обработки'}</div><span>XLSX · CSV/TSV (экспериментально) · до 20 МБ</span></div>
        ...
        {!parsed && canProcessFiles && <div className={`dropzone ${dragging ? 'dragging' : ''}`} onDragOver={event => { event.preventDefault(); setDragging(true) }} onDragLeave={event => { event.preventDefault(); setDragging(false) }} onDrop={event => { event.preventDefault(); setDragging(false); loadFile(event.dataTransfer.files[0]) }}><div className="drop-icon"><UploadCloud size={29} /></div><h3>Перетащите таблицу сюда</h3><p>или выберите XLSX, CSV либо TSV с устройства. Данные обрабатываются только в памяти этой вкладки.</p><input ref={fileInput} type="file" accept=".xlsx,.csv,.tsv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv,text/tab-separated-values" onChange={event => loadFile(event.target.files[0])} aria-label="Выбрать таблицу" /><div className="drop-actions"><button className="button button-primary" onClick={() => fileInput.current?.click()} disabled={busy}>{busy ? 'Открываем файл…' : 'Выбрать файл'} <ArrowRight size={17} /></button><button className="button button-light" onClick={openExample} disabled={busy}>Открыть учебный пример</button></div><small>Учебный пример синтетический; CSV/TSV пока не проверены на рабочих экологических выгрузках.</small></div>}
        {parsed && config && <div className="file-workspace"><div className="file-summary"><div className="file-icon"><FileSpreadsheet size={24} /></div><div><strong>{fileInfo.name}</strong><span>{parsed.sheets.length} {parsed.sheets.length % 10 === 1 && parsed.sheets.length % 100 !== 11 ? 'лист' : parsed.sheets.length % 10 >= 2 && parsed.sheets.length % 10 <= 4 && (parsed.sheets.length % 100 < 12 || parsed.sheets.length % 100 > 14) ? 'листа' : 'листов'} · {(fileInfo.size / 1024 / 1024).toFixed(2)} МБ · SHA-256 {fileInfo.hash.slice(0, 12)}…</span></div><button className="icon-button" onClick={reset} aria-label="Удалить файл из памяти" title="Удалить файл из памяти"><X size={18} /></button></div>{parsed.importInfo && <p className="import-note">Текстовая таблица: {parsed.importInfo.encoding}, разделитель {parsed.importInfo.delimiter}. Адрес A2 означает запись 2; физическая строка исходного файла указана в замечаниях. Проверьте заголовки и значения в предпросмотре.</p>}<div className="mapping-layout"><SheetMapping type="sources" title="Таблица источников" description="Одна строка — один источник. Повтор номера будет отмечен как замечание." fields={SOURCE_FIELDS} sheets={parsed.sheets} workbook={parsed.workbook} config={config.sources} onChange={sources => { setConfig({ ...config, sources }); setResult(null) }} /><SheetMapping type="emissions" title="Таблица выбросов" description="Свяжем вещества с источниками по номеру, без пересчёта величин." fields={EMISSION_FIELDS} sheets={parsed.sheets} workbook={parsed.workbook} config={config.emissions} onChange={emissions => { setConfig({ ...config, emissions }); setResult(null) }} optional /></div>{mode === 'compare' && <ComparisonPanel inputRef={comparisonInput} parsed={comparisonParsed} file={comparisonFile} config={comparisonConfig} confirmed={scopeConfirmed} busy={busy} onLoad={loadComparisonFile} onRemove={removeComparisonFile} onMapping={value => { setComparisonConfig(value); setResult(null) }} onConfirm={value => { setScopeConfirmed(value); setResult(null) }} />}<div className="run-row"><p><LockKeyhole size={16} /> Маппинг и файл остаются только в памяти вкладки. Ничего не сохраняется автоматически.</p><button className="button button-primary" onClick={runValidation}>{mode === 'compare' ? 'Сравнить два файла' : 'Проверить данные'} <ArrowRight size={18} /></button></div></div>}
